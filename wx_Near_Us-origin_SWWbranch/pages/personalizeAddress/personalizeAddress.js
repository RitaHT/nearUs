...
Page({
  ...
  itemAddressClick: function(e){
    ...
    var item = that.data.resultList[Number(e.currentTarget.id)];//get item in resultList
    var lati=item.location.lat;
    var lngt=item.location.lng;
    consoleUtil.log(item);
    //将数据设置到地图页面
    var pages = getCurrentPages();
    var prePage = pages[pages.length - 2];
    //console.log('BEFORE', prePage.data.locationMarkers);
    prePage.setData({
      //callbackAddressInfo: item
      'locationMarkers[0].latitude':lati,
      'locationMarkers[0].longitude':lngt,
      scale: 14,
      longitude:lngt,
      latitude:lati,
      personalizedCallback:true
      //showConfirm:false
    })
    console.log("hhsifhwifhewi")
    //console.log('AFTER', prePage.data.locationMarkers);
    //var addressinfo=item.location;
    //var lati=addressinfo.lat;
    //var lngt=addressinfo.lng;
    //console.log(lati)
    wx.navigateBack({
